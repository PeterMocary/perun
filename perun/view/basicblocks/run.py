...
def sunburst(df, type='time'):
    ...
    big_angle = 2.0 * np.pi / (len(df) + 1)  # +1 for the anotations column
    ...
    # fclx_text = [-30, -30, -30, -30, -30, 40, 40, 40, 40, 40]
    # fcly = [40, 20, 0, -20, -40, 40, 20, 0, -20, -40]
    # p.circle(fclx_circle, fcly, color=list(percentage_color.values()), radius=5)
    # p.text(fclx_text, fcly, text=["<= " + gr + "%" for gr in percentage_color.keys()],
...
@pass_profile
def basicblocks(profile, top_functions, top_basic_blocks, sort_by):
    ...
    # # another VIZ for BBLs
    # sns.set_theme(style="white", rc={"axes.facecolor": (0, 0, 0, 0)})
    # #Create the data
    # func = "QuickSortBad"
    #
    # data = convert.resources_to_pandas_dataframe(profile)
    # data = data[data.uid.str.match("^BBL#QuickSortBad#")]
    #
    # sns.barplot(data=data, x="uid", y="amount")
    # # Initialize the FacetGrid object
    # pal = sns.cubehelix_palette(12, rot=-.25, light=.7)
    # g = sns.FacetGrid(data, row="uid", hue="uid", aspect=15, height=.5, palette=pal)
    #
    # # Draw the densities in a few steps
    # g.map(sns.kdeplot, "amount",
    #     bw_adjust=1, clip_on=False,
    #     fill=True, alpha=1, linewidth=1.5)
    # #g.map(sns.kdeplot, "amount", clip_on=False, color="w", lw=2, bw_adjust=.5)
