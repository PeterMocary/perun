...
class FunctionData:
    ...
    def __init__(self, name: str, time: int = 0, execs: int = 0,
                 basic_blocks: Optional[List[BasicBlockData]] = None) -> None:
        self.name = name
        self.time = time
        self.execs = execs
    ...
    def sort_basic_blocks(self, sort_by: str = 'time') -> None:
        self.basic_blocks = sorted(self.basic_blocks,
...
def convert_to_internal_representation(data: pd.DataFrame) -> List[FunctionData]:
    ...
    for func_name in function_names:
        ...
        for bbl_name in bbl_names:
            ...
            bbl: BasicBlockData = BasicBlockData(bbl_time, bbl_execs,
                                                 bbl_src_lines, bbl_src_file)
            bbls.append(bbl)
...
def extract_relevant_data_to_internal_representation(profile: Profile,
                                                     top_functions: int, top_basic_blocks: int,
                                                     sort_by: str):
    data: pd.DataFrame = convert.resources_to_pandas_dataframe(profile)

    # drop unnecessary columns
    data.drop(columns=['type', 'subtype', 'time', 'workload', 'tid', 'snapshots'],
              inplace=True, axis=1)

    if data.loc[data['uid'] == 'main'].empty:
        # FIXME: exception
        raise Exception("Couldn't create graphs, not enough data. Profile doesn't include main "
                        "function.")
    ...
    function_data = sorted(
        function_data,
        key=lambda func: func.exclusive_time if sort_by == 'time' else func.execs
    )
...
def create_sunburst_graph(function_data, type='time'):
    ...
    p.text([-100, 80], [-20, -20], text=['0%', '100%'], text_font_size='10px',
    ...
    my_table = DataTable(source=source, columns=columns, fit_columns=True,
                         width=1950, height=27*(len(function_data)+1))
    return p, my_table
...
def basicblocks(profile: Profile, top_functions: int, top_basic_blocks: int, sort_by: str):
    ...
    data = extract_relevant_data_to_internal_representation(profile, top_functions,
                                                            top_basic_blocks, sort_by)
    ...
    for function in data:
        print(f"\n{'function name' :<20}{'BBL' :^5}{'lines' :^40}"
              f"{'runtime%' :^10}{'runtime':^12}")
        for idx,bbl in enumerate(function.basic_blocks):
            ...
            print(f"{function.name :<20}{idx:^5}{', '.join([str(l) for l in bbl.src_lines]):^40}"
